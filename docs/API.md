...
| `/api/situational` | Optional USGS earthquake + NASA FIRMS detections clipped to the **region pack bbox** (`web/region.json`). **Disabled by default** (`SITUATIONAL_LAYERS_ENABLED`); not official Sachet alerts. |
| `/api/jev/shadow` | Ops-only shadow Jev / TypeSafe System One triage log (`JEV_SHADOW_ENABLED`, default off). Does not alter `/api/alerts` records. Includes an ops-only badge payload; Jev judgments never appear on public CAP alerts. |
