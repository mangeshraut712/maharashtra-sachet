...
If SACHET is `stale` with `errorCode: http_error` while IMD/INCOIS stay healthy, check Worker logs for `SACHET session|RSS|CAP HTTP`. CAP XML (`rss_maharashtra.xml` and `FetchXMLFile`) is the bulletin authority. NDMA's `FetchPolygonXMLFile` has been observed returning **HTTP 403** (`<error><status>403</status><uri>/cap_public_website/FetchPolygonXMLFile</uri></error>`) from ordinary HTTPS GET; session cookies and a browser User-Agent do not change it. The collector treats that as missing map geometry and must still ingest CAP. No Cloudflare dashboard setting unblocks that NDMA path. Map view then uses district points. If **session, RSS, or CAP** themselves 403 from Cloudflare egress, that is an upstream/WAF block: keep last-good D1 data, point people at [sachet.ndma.gov.in](https://sachet.ndma.gov.in/), and do not invent alerts.
