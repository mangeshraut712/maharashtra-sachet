...
test('Node runner binds ephemeral local listener and safely serves assets', async t => {
  ...
  const moduleScript = await fetch(`${base}/vendor/maplibre-gl.mjs`)
  assert.equal(moduleScript.status, 200)
  assert.match(moduleScript.headers.get('content-type') || '', /javascript/)
  const shared = await fetch(`${base}/vendor/maplibre-gl-shared.mjs`)
  assert.equal(shared.status, 200)
  assert.match(shared.headers.get('content-type') || '', /javascript/)
  const worker = await fetch(`${base}/vendor/maplibre-gl-worker.mjs`)
